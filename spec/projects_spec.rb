...
describe "Ticketmaster::Provider::Kanbanpad::Project" do
  ...
  before(:each) do
    @ticketmaster = TicketMaster.new(:kanbanpad, :token => {})
  ...
  it "should be able to load projects from an array of ids" do
    @projects = @ticketmaster.projects([@project_slug])
    @projects.should be_an_instance_of(Array)
    @projects.first.should be_an_instance_of(@klass)
    @projects.first.slug.should == @project_slug
  end
  
  it "should be able to load all projects from attributes" do
    @projects = @ticketmaster.projects(:slug => @project_slug)
    @projects.should be_an_instance_of(Array)
    @projects.first.should be_an_instance_of(@klass)
    @projects.first.slug.should == @project_slug
  end
  ...
  it "should be able to find a project" do
    ...
    @ticketmaster.project.find(@project_slug).should be_an_instance_of(@klass)
  end
  
  it "should be able to find a project by slug" do
    @ticketmaster.project(@project_slug).should be_an_instance_of(@klass)
    @ticketmaster.project(@project_slug).slug.should == @project_slug
  end
